...
def generate_beta_report(output_name, all_read_data_names, all_read_data):
    ...
    # Genus level heatmap comparison between samples
    gen_data = []
    gen_y_labels = []
    for genus in sorted_zs[0].keys():
        gen_y_labels.append(genus)
        row = []
        for sample_zs in sorted_zs:
            row.append(sample_zs[genus])
        gen_data.append(row)
    gen_x_labels = sorted_zs_names
    
    # Plot z-scores
    while (len(gen_data) > 0):
        top_50_gen_data = gen_data[:50]
        gen_data = gen_data[50:]
        top_50_gen_y_labels = gen_y_labels[:50]
        gen_y_labels = gen_y_labels[50:]
        
        fig, plots = plt.subplots(1, 1)
        fig.set_figwidth(12)
        fig.set_figheight(8)
        im = plots.imshow(top_50_gen_data, aspect='auto', cmap='hot')
        plots.set_title("Genus Z-Scores")
        plots.set_xticks(np.arange(len(gen_x_labels)), labels=gen_x_labels, rotation=45, size='small')
        plots.set_yticks(np.arange(len(top_50_gen_y_labels)), labels=top_50_gen_y_labels)
        plots.figure.colorbar(im)
        plt.tight_layout()
        output.savefig()
        plt.close()
